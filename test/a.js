(function (window) {
    ...
    var lunarCalendar = {
        ...
        leapMonth: function (y) {
            if (!this.lunarInfo["lunar-" + y]) this.getData(y);
        ...
        toZodiac: function (y, m, d) {
            return nStr5[m - (d < this.solarTerms["lunar-" + y][m * 2 - 1] ? 1 : 0)] + "\u5ea7"; //座
        },
        toGanZhiYear: function (lYear) {
            return this.toGanZhi(lYear - 4);
        },
        toGanZhiMonth: function (jd) {
            let y = JD2GL(jd)[0];
            let JZ = Math.round(JD(1998, 12, 7));//以1998年12月7日大雪甲子进行推算
            let sterminof = this.solarTerms["lunar-" + y].slice();
            let jq0 = sterminof[0];
        ...
        getCnZodiac: function (y) {
            return CnZodiac[(y - 4) % 12];
        },
        getSolarTerm: function (jd) {
            let y = JD2GL(jd)[0];
            if (!this.solarTerms["lunar-" + y]) this.getData(y);
            let thisTerm = this.solarTerms["lunar-" + y].slice(1);
            let idx = thisTerm.indexOf(jd);
            if (idx > -1) {
        ...
        solar2lunar: function (y, m, d) {
            ...
            var ly = +ldate[0].split("-")[0],
                lm = +ldate[0].split("-")[1],
                ld = +ldate[0].split("-")[2];
            if (ly != y) this.getData(ly);
            nWeek = getWeek(JD(y, m, d));
            nWeek = (nWeek + 7) % 7;
            cWeek = nStr1[nWeek];
            nWeek = nWeek == 0 ? 7 : nWeek;
            Term = this.getSolarTerm(y, m, d);
        ...
        lunar_yueLi: function (ly, lm, isleap = 0) {
            if (!this.lunarInfo["lunar-" + ly]) this.getData(ly);
            let key = ly + "-" + lm;
        ...
        solar_yueLi: function (y, m) {
            ...
            nWeek = getWeek(ym_jd);
            let isLeap = lunInfo[n][3];
            let cMonthDX = lunInfo[n][2] == 30 ? '大' : '小';
            for (var d = 0; d < yue_days; d++) {
                ...
                if (ld > lunInfo[n][2]) {
                    ...
                    cM = lunInfo[n][3] ? "\u95f0" + cM : cM;
                    isLeap = lunInfo[n][3];
                    cMonthDX = lunInfo[n][2] == 30 ? '大' : '小';
                }
                nWeek += d;
                cWeek = nStr1[nWeek];
                Term = this.getSolarTerm(cday_jd);
                yueli_info = {
                    'lunarYearCn': cY,
                    'lunarMonthCn': cM,
                    'lunarDayCn': this.toChinaDay(ld),
                    'lunarMonthDX': cMonthDX,
                    'lunarYear': ly * 1,
                    'lunarMonth': lm * 1,
                    'lunarDay': ld,
                    'year': y,
        ...
        get_dayInfo: function (y, m, d, ly) {
            var jd = Math.round(JD(y, m, d), 0);
            var nWeek = getWeek(jd);
            if (nWeek == 0) nWeek = 7;
            var zodiacCn = this.getCnZodiac(ly);
            var gzY = this.toGanZhiYear(ly);
            var gzM = this.toGanZhiMonth(jd);
            var dc = JD(y, m, d) - JD(2000, 1, 7) + 900000;
            var gzD = this.toGanZhi(dc);
            var zodiac = this.toZodiac(y, m, d);
            var day_info = {
                'zodiacCn': zodiacCn,
                'gzYear': gzY,
                'gzMonth': gzM,
        ...
        get_24jq: function (year) {
            if (!this.lunarInfo["lunar-" + year]) this.getData(year);
            let _obj = {};
            var solarDays = this.solarTerms['lunar-' + year].slice(1);
            solarDays.forEach((n, i) => {
                let [y, m, d] = JD2GL(n);
                if (y == year) {
                    let key = y + '-' + m.toString().padStart(2, '0') + '-' + d.toString().padStart(2, '0');
                    _obj[key] = SolarTerms[i % 24];
                }
            });
            return _obj;
        }
