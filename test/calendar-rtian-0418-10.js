...
var calendar = {
	...
	get_lunInfo: function (y) {
		...
		for (let i = -7; i < 23; i++) {
			let a = getjq_24(y, i)
			let b = Math.round(getjq_12(y, a), 0)
			arr.push([a, b, false])
			d[Math.round(a, 0)] = i + 7
		}
		console.log(arr)
		if (!this.sTermInfo["lunar-" + y]) { //设置24节气信息
			let termarr = arr.slice(2, 26)
			termarr = termarr.map(function (n) {
				return JD2GL(n[0])[2]
			})
			if (y <= 1582) {
				termarr = arr.slice(1);
	...
	leapYears: function (jd) { //3200/6400/9600减闰
		// return jd < 5227447 ? (jd < 4058671 ? (jd < 2889895 ? 0 : 1) : 2) : 3
		return 0;
	},
	...
	solarDays: function (y, m) {
		...
		var ms = m - 1;
		if (ms == 1) {//3200/6400/9600减闰
			// return (((y % 4 == 0) && (y % 100 != 0) || (y % 400 == 0) && (y % 3200 != 0)) ? 29 : 28);
	...
	toConste: function (y, m, d) {
		return this.nStr5[m - (d < this.sTermInfo["lunar-" + y][m * 2 - 1] ? 1 : 0)] + "\u5ea7"; //座
	},
	...
	toGanZhi: function (offset) {
		return (this.Gan[(offset + 120000) % 10] + this.Zhi[(offset + 120000) % 12]);
	},
	getTerm: function (y, n) {
		if (!this.sTermInfo["lunar-" + y]) this.get_lunInfo(y)
		return parseInt(this.sTermInfo["lunar-" + y][n - 1]);
	},
	toChinaYear: function (y) {
		var cyear = y.toString().split("").map(function (n) {
			return calendar.nStr1[n == 0 ? 10 : n]
		}).join("")
		return cyear + "\u5e74"
	},
	...
	toChinaDay: function (d) {
		if (d % 10 == 0) {
			return this.nStr4[d / 10 - 1]
		} else {
			let n1 = d % 10,
				n2 = parseInt(d / 10)
			return this.nStr2[n2] + this.nStr1[n1]
		}
	},
	...
	solar2lunar: function (y, m, d) {
		...
		var ly = +ldate[0].split("-")[0],
			lm = +ldate[0].split("-")[1],
			ld = +ldate[0].split("-")[2] - this.leapYears(ymd_jd);
		if (ly != y) this.get_lunInfo(ly)
		nWeek = getWeek(JD(y, m, d));
		nWeek = (nWeek + 7 - this.leapYears(ymd_jd)) % 7
		cWeek = this.nStr1[nWeek];
		nWeek = nWeek == 0 ? 7 : nWeek;
		isTerm = false;
		Term = null;
		var Tn = this.sTermInfo["lunar-" + y].slice(m * 2 - 2, m * 2).indexOf(d)
		if (Tn > -1) isTerm = true, Term = this.solarTerm[m * 2 - 2 + Tn];
	...
	lunar2solar: function (y, m, d, isLeapMonth) {
		...
		var s_day = JD2GL(ymd[1] + d - 1 + this.leapYears(ymd[1] + d)),
			yy = +s_day[0],
			mm = +s_day[1],
			dd = +s_day[2]
	...
	solar_yueLi: function (y, m) {
		if (!this.lunarInfo["lunar-" + y]) this.get_lunInfo(y)
		var cd = JD(y, m, 1)
		var cd_week = getWeek(cd)
		var cdate = new Date(y, m - 1, 2 - cd_week)
		var lunInfo = this.lunarInfo["lunar-" + y].month
		var yueli = [],
		...
		ld -= this.leapYears(ym_jd)
		cY = this.toChinaYear(ly)
		cM = this.toChinaMonth(lm)
		cM = lunInfo[n][3] ? "\u95f0" + cM : cM
		var cY, cM, cD, cDate, nWeek, cWeek, firstNode, secondNode, isTerm, Term, yueli_info, d_info
		for (var d = 1; d <= yue_days; d++) {
			if (y == 1582 && m == 10 && (d > 4 && d < 15)) {
				//
				console.log("跳过1582年10月4日-10月14日")
				continue;
			}
			ld++
			if (ld > lunInfo[n][2]) {
				...
				ly = lun_ymd[0], lm = lun_ymd[1], ld -= lunInfo[n - 1][2]
				cY = this.toChinaYear(ly)
				cM = this.toChinaMonth(lm)
				cM = lunInfo[n][3] ? "\u95f0" + cM : cM
			}
			// cDate = new Date(y, parseInt(m) - 1, d)
			cDate = JD(y, m, d)
			nWeek = getWeek(cDate);
			cWeek = this.nStr1[nWeek];
			// if (nWeek == 0) nWeek = 7;
			...
			Term = null;
			var term_i = this.sTermInfo["lunar-" + y].slice(m * 2 - 2, m * 2).indexOf(d)
			if (term_i > -1) isTerm = true, Term = this.solarTerm[m * 2 - 2 + term_i];
			yueli_info = {
				'lYear': ly * 1,
				'lMonth': lm * 1,
				'lDay': ld,
			...
			d_info = this.get_dayInfo(y, m, d, ly)
	...
	get_dayInfo: function (y, m, d, ly) {
		...
		var gz_jq = this.sTermInfo["lunar-" + y][m * 2 - 2];
		var gzM = this.toGanZhi((y - 1900) * 12 + m + (d >= gz_jq ? 12 : 11)); // 依据12节气修正干支月
		var dc = Date.UTC(y, m - 1, 1, 0, 0, 0, 0) / 86400000 + 25567 + 10; //有BUG，1582年10月4日-10月14日
		if (y == 1582 && m == 10 && d < 15) {
			//
			dc += 10;
		}
		var gzD = this.toGanZhi(dc + d - 1);
